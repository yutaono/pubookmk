...
(function() {
  $(function() {
    ...
    panelHover = function() {
      return $("#bookmarkGrid .panel").hover((function() {
        $(this).find(".panelFooter").css({
          display: "inline"
        });
        return $(this).find(".panelFooter").animate({
          height: "52px"
        }, 400);
      }), function() {
        return $(this).find(".panelFooter").animate({
    ...
    panelHover();
    $("input#bookmark_url").on("keyup", function() {
      var isUrl;
      isUrl = $(this).val().match(/http[s]?\:\/\/[\w\+\$\;\?\.\%\,\!\#\~\*\/\:\@\&\\\=\_\-]+/);
      if (isUrl) {
        return $(".inputUrl p input[type='submit']").css({
    ...
    $(".inputUrl").on('ajax:complete', function(event, ajax, status) {
      var html, res;
      ...
      if (status === "already exist") {
        ...
        $('#bookmarkGrid .panel:first-child').height('0').animate({
          height: '140px',
          duration: 600
        });
        panelHover();
        $(".deleteButton").on('ajax:complete', function(event, ajax, status) {
          var $thisPanel;
          res = $.parseJSON(ajax.responseText);
          status = res.status;
          $thisPanel = $(this).parent().parent().parent();
          return $thisPanel.animate({
            opacity: '0.0',
            height: '0'
          }, {
            duration: 600,
        ...
        return $("#bookmark_url").val("");
      }
    });
    return $(".deleteButton").on('ajax:complete', function(event, ajax, status) {
      var $thisPanel, res;
      res = $.parseJSON(ajax.responseText);
      status = res.status;
